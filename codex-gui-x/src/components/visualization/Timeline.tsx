  ...
  return (
    <div className="w-full p-4 bg-gray-800/50 backdrop-blur-lg rounded-xl border border-gray-700">
      ...
      <div className="relative">
        <div
          ...
          onClick={(e) => {
            ...
            const newIndex = Math.floor(percentage * commits.length)
            onSeek(Math.max(0, Math.min(newIndex, commits.length - 1)))
          }}
        >
          <div
        ...
        <div className="flex justify-between mt-2 text-xs text-gray-500">
          <span>{new Date(commits[0]?.timestamp || 0).toLocaleDateString()}</span>
          <span>{new Date(commits[commits.length - 1]?.timestamp || 0).toLocaleDateString()}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 mt-4">
        ...
        />
        <label htmlFor="loop" className="text-sm text-gray-400">
          Loop playback
        </label>
      </div>
    </div>
