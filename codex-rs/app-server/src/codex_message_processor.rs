...
impl CodexMessageProcessor {
    ...
    async fn upload_feedback(&self, request_id: ConnectionRequestId, params: FeedbackUploadParams) {
        ...
        let _sqlite_feedback_logs = if include_logs {
        ...
        let rollout_attachment_path = attachment_paths.first().cloned();
        let upload_result = tokio::task::spawn_blocking(move || {
            snapshot.upload_feedback(
                &classification,
                reason.as_deref(),
                include_logs,
                rollout_attachment_path.as_deref(),
            )
        })
        .await;
