...
impl Microsoft365Client {
    ...
    async fn make_request(
        ...
        if !status.is_success() {
            ...
            return Err(anyhow::anyhow!(
                "Microsoft 365 API error ({status}): {error_text}"
            ));
        }
    ...
    pub async fn word_read_document(&self, drive_id: &str, item_id: &str) -> Result<String> {
        let endpoint = format!("/drives/{drive_id}/items/{item_id}/content");
    ...
    pub async fn excel_read_spreadsheet(
        &self,
        drive_id: &str,
        item_id: &str,
    ) -> Result<serde_json::Value> {
        let endpoint = format!("/drives/{drive_id}/items/{item_id}/workbook");
    ...
    pub async fn excel_update_cell(
        ...
        let endpoint = format!(
            "/drives/{drive_id}/items/{item_id}/workbook/worksheets/{worksheet}/range(address='{cell}')"
        );
    ...
    pub async fn powerpoint_read_presentation(
        &self,
        drive_id: &str,
        item_id: &str,
    ) -> Result<serde_json::Value> {
        let endpoint = format!("/drives/{drive_id}/items/{item_id}");
        self.make_request("GET", &endpoint, None).await
    }
    ...
    pub async fn outlook_get_calendar_events(
        ...
        let endpoint = format!("/me/calendar/calendarView?startDateTime={start}&endDateTime={end}");
        let response = self.make_request("GET", &endpoint, None).await?;
