...
2.  **Added Missing CLI Dependencies**:
    ...
    - Added `cuda` feature to `cli/Cargo.toml` to resolve `cfg(feature = "cuda")` warnings.
...
The codebase should now be largely free of the initial compliation errors.
...
- If `git_commands.rs` still complains about `cuda`, verify the feature flag propagation.
- Ensure `codex-chrome-mcp-bridge` and `codex-chrome-host` binaries are built/available if `chrome_cmd.rs` logic requires them at runtime (though compile-time should now be fine).
