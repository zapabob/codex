...
use crate::kernel_driver_ffi::PinnedMemory;
...
impl KernelCudaBridge {
    ...
    pub fn allocate_pinned_memory(&mut self, size: u64) -> Result<()> {
        ...
        let pinned = PinnedMemory::new(handle, size as usize)
        ...
        if let Some(cuda) = &self.cuda_runtime {
            debug!(
                "Registering pinned memory with CUDA runtime (device {})",
                cuda.device_id()
            );
    ...
    pub fn optimize_scheduling(&self) -> Result<()> {
        ...
        info!(
            ...
            stats.ai_task_count, stats.boosted_thread_count, stats.average_latency_ms
        );
