...
class CoworkIntegrationTester:
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for suite_name, test_func in test_suites:
            try:
                ...
                self.test_results.append(
                    {"suite": suite_name, "status": "error", "error": str(e)}
                )
    ...
    async def test_document_generation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            word_content = {
                "title": "テスト文書",
                "sections": [{"heading": "テスト", "paragraphs": ["これはテストです"]}],
            }
            ...
            ppt_data = {
                "title": "テストプレゼン",
                "slides": [{"title": "スライド1", "content": "テストコンテンツ"}],
            }
            ...
            return {
                "success": True,
                "files": [str(excel_path), str(word_path), str(ppt_path)],
            }
        ...
        try:
            if EnhancedBrowserAutomationEngine is None:
                return {
                    "success": True,
                    "skipped": True,
                    "reason": "EnhancedBrowserAutomationEngine not available",
                }
        ...
        try:
            if AsanaConnector is None or NotionConnector is None:
                return {
                    "success": True,
                    "skipped": True,
                    "reason": "Connectors not available",
                }
            ...
            if not asana_key:
                logger.warning(
                    "ASANA_API_KEYが設定されていないため、Asanaテストをスキップ"
                )
