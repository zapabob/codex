...
class MCPServerTester:
    ...
    def add_result(self, test_name, status, output=""):
        self.test_results.append(
            {"test": test_name, "status": status, "output": output}
        )
        if status == "PASS":
            self.pass_count += 1
        else:
    ...
    def test_mcp_server_startup(self):
        ...
        try:
            ...
            if proc.poll() is None:
                ...
                self.add_result(
                    "MCP Server Startup", "PASS", "Server started and running"
                )
                ...
                self.add_result(
                    "MCP Server Startup", "FAIL", f"Server exited: {stderr}"
                )
                return False
    ...
    def test_mcp_tools_list(self):
        ...
        try:
            ...
            if proc.poll() is None:
                ...
                self.add_result(
                    "MCP Tools List", "PASS", "Server accepts tools/list method"
                )
                proc.terminate()
                proc.wait(timeout=5)
                return True
    ...
    def test_mcp_binary_size(self):
        ...
        if os.path.exists(binary_path):
            ...
            if size_mb > 30:
                ...
                self.add_result(
                    "MCP Binary Size", "FAIL", f"Size too small: {size_mb:.2f} MB"
                )
                return False
        else:
    ...
    def print_summary(self):
        ...
        if self.fail_count == 0:
            print(
                f"{Colors.GREEN}{Colors.BOLD}Overall Status: ALL MCP JSONRPC TESTS PASSED!{Colors.RESET}"
            )
        elif self.fail_count <= 2:
            print(
                f"{Colors.YELLOW}Overall Status: MOSTLY PASSED (minor issues){Colors.RESET}"
            )
        else:
            print(
                f"{Colors.RED}{Colors.BOLD}Overall Status: TESTS FAILED{Colors.RESET}"
            )
        print()
        ...
        for result in self.test_results:
            ...
            print(
                f"  - {result['test']}: {status_color}{result['status']}{Colors.RESET}"
            )
            if result["output"]:
                output_preview = (
                    result["output"][:80] + "..."
...
"""
        ...
        if self.fail_count == 0:
            ...
            log_content += (
                "❌ **NEEDS ATTENTION** - Multiple tests failed. Please review.\n"
            )
