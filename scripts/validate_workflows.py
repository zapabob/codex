...
for fname in files:
    try:
        ...
        issues = []
        if "@v6" in content:
            issues.append("has @v6 refs: " + str(content.count("@v6")))
        if "codex-runners" in content:
            issues.append("has codex-runners")
        if "macos-15-xlarge" in content:
            issues.append("has macos-15-xlarge")
