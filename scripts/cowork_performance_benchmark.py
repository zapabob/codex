...
class PerformanceBenchmark:
    ...
    async def benchmark_cache_performance(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "speedup": speedup,
            "cache_hit_rate": len([t for t in cache_times if t < 0.001]) / len(cache_times) * 100
        }
    ...
    async def benchmark_parallel_processing(self) -> Dict[str, Any]:
        ...
        for i in range(3):
            start = time.time()
            tasks = [
                self.resource_manager.execute_with_limit(
                    f"task_{j}",
                    dummy_task(j)
                ) for j in range(5)
            ]
    ...
    async def benchmark_document_generation(self) -> Dict[str, Any]:
        ...
        logger.info(f"[OK] ドキュメント生成: Excel {result['excel_avg_ms']:.2f}ms, Word {result['word_avg_ms']:.2f}ms")
        return result
    ...
    async def benchmark_session_management(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            "add_task_avg_ms": statistics.mean(add_task_times) * 1000 if add_task_times else 0,
            "create_min_ms": min(create_times) * 1000,
            "create_max_ms": max(create_times) * 1000
        }
        
        logger.info(f"[OK] セッション管理: 作成 {result['create_session_avg_ms']:.2f}ms")
        return result
    ...
    def print_summary(self, summary: Dict[str, Any]):
        ...
        for name, result in summary["results"].items():
            if "error" in result:
                ...
                print(f"[OK] {name}: 平均 {result.get('avg_ms', result.get('excel_avg_ms', 0)):.2f}ms")
        
        print("-" * 60)
        print(f"キャッシュサイズ: {summary['cache_stats']['size']}/{summary['cache_stats']['max_size']}")
