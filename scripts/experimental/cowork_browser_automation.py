...
class EnhancedBrowserAutomationEngine:
    ...
    async def initialize(self):
        ...
        logger.info(
            f"ブラウザ初期化完了: {self.browser_type} (headless={self.headless})"
        )
    ...
    async def capture_screenshot(
        self, page: Page, element_selector: Optional[str] = None
    ) -> str:
        """スクリーンショット取得"""
    ...
    async def find_element_by_text(
        self, page: Page, text: str, element_type: Optional[str] = None
    ...
    async def click_element(
        self, page: Page, selector: str, wait_for_navigation: bool = False
    ):
        """要素クリック"""
        try:
    ...
    async def fill_form(self, page: Page, form_data: Dict[str, str]):
        ...
        for field_name, value in form_data.items():
            try:
                ...
                if not filled:
                    # テキスト検索で試行
                    visual_element = await self.find_element_by_text(
                        page, field_name, "input"
                    )
                    if visual_element and visual_element.selector:
                        await page.fill(visual_element.selector, value)
                        logger.info(
                            f"フォーム入力（視覚検索）: {field_name} = {value[:20]}..."
                        )
                    else:
    ...
    async def extract_data(
        self, page: Page, extraction_rules: Dict[str, str]
    ) -> Dict[str, Any]:
        """データ抽出（セレクタールールベース）"""
        extracted_data = {}
    ...
    async def execute_workflow(
        self, page: Page, workflow: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """ワークフロー実行"""
        results = []
        ...
        for step in workflow:
            ...
            try:
                if step_type == "navigate":
                    ...
                    results.append(
                        {"step": step_type, "status": "success", "selector": selector}
                    )
                    ...
                    results.append(
                        {"step": step_type, "status": "success", "data": data}
                    )

                elif step_type == "screenshot":
                    path = await self.capture_screenshot(page)
                    results.append(
                        {"step": step_type, "status": "success", "path": path}
                    )
