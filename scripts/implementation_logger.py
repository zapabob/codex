...
def load_recent_implementation_logs(limit: int = 5) -> List[Dict]:
    ...
    logs = []
    for log_file in sorted(docs_dir.glob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True):
        if log_file.is_file():
            try:
                with open(log_file, "r", encoding="utf-8") as f:
                    content = f.read()
                    # 日時を抽出
                    date_match = re.search(r'\*\*日時\*\*:\s*(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})', content)
                    # 機能名を抽出（最初の# の後）
                    ...
                    logs.append({
                        ...
                        "feature": title_match.group(1) if title_match else log_file.stem,
                        "worktree": worktree_match.group(1) if worktree_match else "main",
                        "content": content[:500]  # 最初の500文字
                    })
            except Exception as e:
                print(f"警告: ログファイルの読み込みに失敗: {log_file}: {e}", file=sys.stderr)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("使用方法:")
        print("  python implementation_logger.py create <機能名> <タスク説明> [実装詳細]")
