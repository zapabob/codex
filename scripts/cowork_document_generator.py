...
class DocumentGenerationEngine:
    ...
    def _apply_excel_styles(self, ws, styles: Dict[str, Any]):
        ...
        if "header" in styles:
            ...
            fill = PatternFill(start_color=header_style.get("bg_color", "366092"),
    ...
    def generate_powerpoint(self, output_path: str, presentation_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # テンプレート読み込み（オプション）
            if "template" in presentation_data and Path(presentation_data["template"]).exists():
        ...
        "sections": [
            {
                "heading": "はじめに",
                "level": 1,
                "paragraphs": ["これはサンプル文書です。", "ClaudeCoworkスタイルのドキュメント生成機能をテストしています。"],
                "style": {"bold": False}
            },
            {
                "heading": "機能一覧",
